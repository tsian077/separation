...
outfile = "test_x_hamming_ln.hdf5"

window = signal.hamming(512)

hf = h5py.File(outfile, 'w')
frame = np.zeros((0, 5, 512))
dset = hf.create_dataset('test_x', data=frame, maxshape=(None, 5, 512), chunks=True)
for index, f in enumerate(files):
    ...
    temp = np.log(temp)
    print(temp.shape)
    hf['test_x'].resize((hf['test_x'].shape[0] + temp.shape[0]), axis=0)
    hf['test_x'][-1 * temp.shape[0]:] = temp
