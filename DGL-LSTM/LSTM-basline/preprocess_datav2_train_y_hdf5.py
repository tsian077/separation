...
dir_name = '../trainOrigin'

files = os.listdir(dir_name)
outfile = "train_y_hamming_ln.hdf5"

window = signal.hamming(512)

hf = h5py.File(outfile, 'w')
frame = np.zeros((0, 512))
dset = hf.create_dataset('train_y', data=frame, maxshape=(None, 512), chunks=True)
for index, f in enumerate(files):
    ...
    temp = np.log(temp)
    print(temp.shape)
    hf['train_y'].resize((hf['train_y'].shape[0] + temp.shape[0]), axis=0)
    hf['train_y'][-1 * temp.shape[0]:] = temp
